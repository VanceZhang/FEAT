...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='MiniImageNet',
                        ...
                        help='default is MiniImageNet.other options are TieredImagenet and CUB.')
    parser.add_argument('--backbone_class', type=str, default='Res12', choices=['ConvNet', 'Res12'],
                        help='default is Res12.choose backbone from ConvNet and Res12.')
    parser.add_argument('--schedule', type=int, nargs='+', default=[75, 150, 300],
                        help='Decrease learning rate at these epochs.')
    parser.add_argument('--gamma', type=float, default=0.1)
    parser.add_argument('--query', type=int, default=15)
    parser.add_argument('--resume', type=bool, default=False, help='在已有模型的基础上继续训练')
    ...
    def save_checkpoint(is_best, filename='checkpoint.pth.tar'):
        """
        保存检查点,包括模型参数，优化器参数，训练日志等
        如果是最好的模型，还会保存一个model_best.pth.tar
        is_best: 是否是最好的模型
        filename: 保存文件名
        """
    ...
    # resume为True时，从最佳模型继续训练
    if args.resume:
        # load checkpoint
        state: dict = torch.load(osp.join(args.save_path, 'model_best.pth.tar'))
    ...
    # 从头开始训练
        ...
        init_epoch = 1
        trlog: dict = {'args': vars(args), 'train_loss': [], 'val_loss_dist': [], 'val_loss_sim': [], 'train_acc': [],
                       'val_acc_sim': [], 'val_acc_dist': [], 'max_acc_dist': 0.0, 'max_acc_dist_epoch': 0,
                       'max_acc_sim': 0.0, 'max_acc_sim_epoch': 0}
        initial_lr = args.lr
        global_count = 0
    ...
    for epoch in range(init_epoch, args.max_epoch + 1):
        ...
        for i, batch in enumerate(train_loader, 1):
            ...
            logits = model(data)  # Classifier
            loss = criterion(logits, label)  # CrossEntropyLoss
        ...
        # 不在前500个epoches计算验证集的准确率，因为这个时候模型还没有收敛
