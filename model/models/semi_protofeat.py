...
class SemiProtoFEAT(FewShotModel):
    ...
    def get_proto(self, x_shot, x_pool):
        ...
        if self.args.use_euclidean:
            dis = - torch.sum((proto_shot.unsqueeze(1).expand(num_batch, num_pool, num_way, emb_dim).contiguous().view(
                num_batch * num_pool, num_way, emb_dim) - x_pool.view(-1, emb_dim).unsqueeze(1)) ** 2,
                              2) / self.args.temperature
        else:
            dis = torch.bmm(x_pool.view(num_batch, -1, emb_dim),
                            F.normalize(proto_shot, dim=-1).permute([0, 2, 1])) / self.args.temperature
        ...
        h = torch.cat([x_shot.view(num_batch, -1, emb_dim), x_pool.view(num_batch, -1, emb_dim)],
                      dim=1)  # (num_batch, n_shot + n_pool, n_embedding)
    ...
    def _forward(self, instance_embs, support_idx, query_idx):
        ...
        support = self.slf_attn(support.view(num_batch, -1, emb_dim), whole_set, whole_set).view(num_batch, num_shot,
                                                                                                 num_way, emb_dim)
